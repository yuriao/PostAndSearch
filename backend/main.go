...
func main() {
	...
	r.Handle("/", jwtMiddleware.Handler(http.HandlerFunc(startupTestHandler))).Methods("GET", "OPTIONS")
	...
	//r.Handle("/", http.HandlerFunc(startupTestHandler)).Methods("GET", "OPTIONS")
	...
	//r.Handle("/post/{id}", http.HandlerFunc(deleteHandler)).Methods("DELETE", "OPTIONS")
